...
namespace FreshSzalonAdmin
{
    public partial class VendegKorlatozasForm : MaterialForm
    {
        ...
        {
            System.Data.DataTable dt = adatbazis.GetVendegJogok(kivalasztottId);

            if (dt.Rows.Count > 0)
            {
                swVelemeny.Checked = Convert.ToInt32(dt.Rows[0]["velemenyt_irhat"]) == 1;
                swFoglalas.Checked = Convert.ToInt32(dt.Rows[0]["foglalhat"]) == 1;
            }
        }
        ...
        {
            int velemeny = swVelemeny.Checked ? 1 : 0;
            int foglalas = swFoglalas.Checked ? 1 : 0;

            adatbazis.FrissitVendegJogok(kivalasztottId, velemeny, foglalas);

            MessageBox.Show("A vendég jogai sikeresen frissítve lettek!", "Siker", MessageBoxButtons.OK, MessageBoxIcon.Information);
            this.Close();
        }
    }
}
