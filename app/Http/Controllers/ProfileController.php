...
class ProfileController extends Controller
{
    public function index()
    {
        ...
        $maiDatum = now()->format('Y-m-d');
        $pontosIdo = now()->format('H:i:s');

        // --- 1. AKTUÁLIS FOGLALÁSOK ---
        // Jövőbeli napok VAGY (Mai nap ÉS még nem járt le az időpont)
        // ÉS a státusz nem "Elvégezve" (4)
        $aktualis_foglalasok = Idopontfoglalas::where('felhasznalo_id', $felhasznalo->id)
            ->where('statuszok_id', '!=', 4)
            ->where(function ($query) use ($maiDatum, $pontosIdo) {
                $query->where('datum', '>', $maiDatum)
                      ->orWhere(function ($sub) use ($maiDatum, $pontosIdo) {
                          $sub->where('datum', '=', $maiDatum)
                              ->where('ido_vege', '>', $pontosIdo);
                      });
            })
            ->with(['szolgaltatas', 'dolgozo', 'statusz']) 
            ->orderBy('datum')
            ->orderBy('ido_kezdes')
            ->get();

        // --- 2. KORÁBBI FOGLALÁSOK ---
        // Múltbeli napok VAGY (Mai nap ÉS már lejárt az időpont)
        // VAGY a státusz már "Elvégezve" (4)
        $korabbi_foglalasok = Idopontfoglalas::where('felhasznalo_id', $felhasznalo->id)
            ->where(function ($query) use ($maiDatum, $pontosIdo) {
                $query->where('datum', '<', $maiDatum)
                      ...
                      ->orWhere('statuszok_id', 4); // <-- Ha a dolgozó "Elvégezve" gombra nyom, azonnal ide kerül!
            })
            ->with(['szolgaltatas', 'dolgozo', 'statusz', 'velemeny']) 
            ->orderBy('datum', 'desc')
            ->orderBy('ido_kezdes', 'desc')
            ->get();
